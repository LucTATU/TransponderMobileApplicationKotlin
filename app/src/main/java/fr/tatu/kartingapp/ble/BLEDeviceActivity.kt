...
class BLEDeviceActivity : AppCompatActivity() {
    ...
    private fun onConnectionStateChange(newState: Int, gatt: BluetoothGatt?) {
        BLEConnexionState.getBLEConnexionStateFromState(newState)?.let {
            runOnUiThread {
                ...
                deviceStatus.text =getString(R.string.ble_device_status, getString(it.text))
            }
    ...
    private fun onServicesDiscovered(gatt: BluetoothGatt?) {
        ...
        gatt?.services?.let {
            runOnUiThread {
                adapter = BLEServiceAdapter(this,
                    it.filter { it.uuid.toString() == uuidServiceWanted } //in order to have only uuid service that we want
                    ...
                    { characteristic, enable -> toggleNotificationOnCharacteristic(gatt,characteristic,enable) }
                )
                bleServicesList.adapter = adapter
                bleServicesList.addItemDecoration(DividerItemDecoration(this@BLEDeviceActivity,LinearLayoutManager.VERTICAL))
            }
        }
    }
